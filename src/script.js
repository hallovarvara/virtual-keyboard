...
//   ['ControlLeft', 'AltLeft', 'MetaLeft', 'Space', 'MetaRight', 'ArrowLeft', 'ShiftRight', 'ArrowRight', 'AltRight'],
// ];
  ...
  createKeyboard() {
    ...
    KEYS.forEach((rowData) => {
      ...
      rowData.forEach((key) => {
        ...
        if (key.isSpecial) button.innerHTML = key.name;
        else if (this.shiftPressed) button.innerHTML = key[`${this.language}Shift`];
        else if (this.capslockPressed) button.innerHTML = (key.length === 1) ? key[this.language].toUpperCase() : key[this.language];
        else button.innerHTML = key[this.language];

        if (key.classes !== undefined) button.classList = key.classes;
        row.append(button);
      });
      keyboard.append(row);
    });
